...
void Program::setupOpenXr(App* app) {
  ...
  ////// Create the instance
  {
    ...
    strcpy(instanceCreateInfo.applicationInfo.applicationName, "App Realimentacion");
    ...
    checkXr(
      ...
      for (const auto& mode : blendModes) {
          if (mode == XR_ENVIRONMENT_BLEND_MODE_ADDITIVE) {
              _xrBlendMode = mode;
              foundAdditive = true;
              LOG_INFO("Modo de realidad mixta (Additive) seleccionado");
              break;
          }
      }

      // Si no encuentra additive, usar el primero disponible
      if (!foundAdditive) {
          _xrBlendMode = blendModes.front();
          LOG_INFO("ℹUsando blend mode disponible: %d", _xrBlendMode);
      }
  }
  ...
  ////// Create the action set and actions
  {
    ...
    strcpy(info.actionSetName, "app_realimentacion");
    strcpy(info.localizedActionSetName, "App Realimentacion");
